...
export async function POST(request: Request) {
  try {
    ...
    const to = process.env.CONTACT_EMAIL_RECIPIENT || 'website-submissions@deltahealthcenter.org';
    ...
    // Setup email data
    const mailOptions = {
      from: `"DHC Contact Form" <${user || 'no-reply@deltahealthcenter.org'}>`,
      to,
      subject: `New DHC Website Contact Submission: ${subject}`,
      text: `
Name: ${name}
Email: ${email}
Phone: ${phone || 'N/A'}
...
${message}
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; border: 1px solid #e0e0e0; padding: 20px; border-radius: 8px;">
          <h2 style="color: #007791; border-bottom: 2px solid #007791; padding-bottom: 10px;">New Website Contact Submission</h2>
          ...
          <p style="font-size: 11px; color: #999; margin-top: 30px; border-top: 1px solid #eee; padding-top: 10px;">
            This contact request was submitted via the Delta Health Center website Contact Form.
          </p>
        </div>
      `
    };
    ...
    if (user && pass) {
      ...
      console.log('--- SIMULATED CONTACT EMAIL SEND (No SMTP credentials configured in env) ---');
      console.log('To:', to);
    ...
    console.error('Error in website contact route:', error);
